import { listOrNone, normalizePercent, renderProgressBar, } from "./render-common.js";
export function renderKrBatchFiles(model) {
    return {
...
function renderKrBatchCockpit(model) {
    ...
    const remainingText = progress.remainingTasks === 0
        ? "남은 작업 없음"
...
function renderCurrentBatchDoc(model) {
    const { progress } = model;
    const safePercent = normalizePercent(progress.percentComplete);
    const currentTask = progress.currentTask ?? "현재 배치";
    const warnings = progress.warnings.length === 0
        ? ["- 아직 따로 적을 조심거리는 없습니다."]
...
function renderKoreanStatus(status) {
    switch (status) {
        case "not_started":
            return "대기";
        case "in_progress":
...
function renderKrBatchProgressBar(percent) {
