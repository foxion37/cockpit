...
import { addInlineHtmlDiagnostics, addPlaceholderDiagnostics, countMatches, diagnostic, isInside, lineNumberForText, relativePath, } from "./common.js";
import { REQUIRED_KR_BATCH_SECTIONS } from "./constants.js";
export async function validateCockpitText(text, context) {
...
function validateRequiredSections(text, filePath, diagnostics) {
...
function validateMermaidColorBlocks(text, filePath, diagnostics) {
    const mermaidBlockCount = countMatches(text, /```mermaid\b/giu);
    if (mermaidBlockCount < 3) {
        diagnostics.push(diagnostic({
            code: "mermaid_block_missing",
            filePath,
            message: "At least three Mermaid blocks are required for visible status color.",
...
async function validateBatchLinks(text, lines, context) {
    const filePath = relativePath(context.repoRoot, context.filePath);
...
function validateProgressRows(lines, filePath, diagnostics) {
    ...
    for (const { line, lineNumber } of lines
        ...
        diagnostics.push(diagnostic({
            ...
            filePath,
            lineNumber,
            message: "Progress table delimiters must not right-align cells with ---:.",
            detail: line,
        }));
    }
    ...
    for (const { line, lineNumber } of progressRows) {
        ...
        if (progressCell === undefined || !/^[█░]/u.test(progressCell.trim())) {
            diagnostics.push(diagnostic({
                ...
                filePath,
                lineNumber,
                message: "Progress bars must start at the left edge of the progress cell.",
                detail: line,
            }));
        }
...
function validateCockpitLength(lines, filePath, context) {
