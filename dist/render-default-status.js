import { fixedColumns, normalizePercent, pipeColumns, renderDotCells, renderProgressBar, } from "./render-common.js";
export function renderStatusKr(model) {
    const { progress } = model;
    const safePercent = normalizePercent(progress.percentComplete);
    const currentTask = progress.currentTask ?? "없음";
    const riskText = progress.warnings.length === 0
        ? "큰 경고 없음"
...
function renderKoreanStatusBoard(model, safePercent) {
    const { progress } = model;
    const currentTask = progress.currentTask ?? "없음";
    const riskText = progress.warnings.length === 0
        ? "큰 경고 없음"
    ...
    return [
        ...
        fixedColumns(["리스크", progress.warnings.length === 0 ? "정상" : "주의", riskText], 7),
    ];
}
